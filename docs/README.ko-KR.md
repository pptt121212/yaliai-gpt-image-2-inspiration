...
Yali AI의 GPT-image2 영감 라이브러리를 검색하고, 이미지 생성 프롬프트를 작성하며, Yali Free Image API를 활용하는 Agent Skill입니다. PPT, 슬라이드, 덱 요청은 로컬 PPT 워크플로로 라우팅합니다.
...
- `YALIAI_API_KEY` 설정 시 Yali 이미지 생성 API를 사용하고 결과를 로컬화한 뒤 절대 경로 Markdown으로 표시.
