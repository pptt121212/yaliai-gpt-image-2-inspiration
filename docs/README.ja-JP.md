...
Yali AI の GPT-image2 インスピレーションライブラリを検索し、画像生成プロンプトを作成し、Yali Free Image API または Codex のネイティブ画像生成を使うための Agent Skill です。PPT / スライド / デッキの依頼は、ローカル PPT ワークフローへルーティングします。
...
- `YALIAI_API_KEY` 設定時は Yali 画像生成 API を利用。
- Codex ではネイティブ画像生成機能と連携可能。
